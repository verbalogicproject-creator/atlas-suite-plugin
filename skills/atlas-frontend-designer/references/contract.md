...
- Output: proposed content/CSS diff, deterministic rebind, visual rationale,
  validation receipt
...
`design.css` is the visual refinement artifact.

`index.html` is the compiler-owned hub binding those artifacts through
protected IDs and design handles. The designer never edits it directly.
